...
The remainder of this paper is structured as follows: Section~\ref{background} gives background about foraging in nature and swarm robotics. Section~\ref{sec:PrioritizedForaging} describes the problem of prioritized foraging. Section~\ref{algorithms} describes the swarm robotics foraging algorithms that are selected for the evaluation on the prioritized foraging problem in detail. Section~\ref{methodology} discusses the nature of the robots, as well as the environments that are used in the experiments. Section~\ref{thri:third:performancemeasures} presents the performance measures used to evaluate the algorithms on the prioritized foraging problem. Section~\ref{results} presents and discusses the results of the experiments. Finally conclusions and future research are presented in Section~\ref{Conclusions}.
...
This section provides background on foraging in swarm robotics.  Section~\ref{swarmrobotics} defines swarm robotics and its inspirations. Section~\ref{foraginginnature} describes foraging behaviour of ants and bees in nature, while existing research for foraging in swarm robotics is outlined in Section~\ref{foragerrobots}. Finally, division of labour in swarm robotics is discussed in  Section~\ref{background:divisionoflabour:robotswarm}.
...
\Function{forage}{$role, state, v, \varsigma, i, \omega, \mu_\varsigma$}
	...
	\State \text{robot moves a step in direction $\sigma$}
	\If {\text{(path integration complete) or (robot sees item of type $\varsigma$)}}
...
The performance of swarm robotics algorithms can be measured by examining the algorithm's ability to perform a task in terms of efficiency, scalability, flexibility, and robustness. Section~\ref{setup:foragingefficiency} presents how efficiency is measured, while a measures for determining flexibility are defined in Section~\ref{setup:flexibility}. The measures for evaluating scalability on an algorithm is described in Section~\ref{setup:scalability}, and Section~\ref{behaviouralperformancemeasures} discusses behavioural performance measures specific to the honey bee algorithm.
...
Redundancy in a swarm is achieved by giving all, or a portion of the robots in the swarm the same capabilities. In this way, if a percentage of the swarm malfunctions, the other robots have the capability to take the malfunctioning robots' place. A swarm demonstrates the highest level of redundancy when any robot can take on the role of any other robot that malfunctions in a swarm. 
...
In summation, this study primarily contributed the prioritized foraging problem and the novel honey bee inspired foraging swarm robotic algorithm. The emperical analysis provided a detailed view of how well each algorithm embodied each major characteristic of swarm robotics on the prioritized foraging problem.
