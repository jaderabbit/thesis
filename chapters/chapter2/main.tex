...
This section discusses some foraging algorithms that have been developed and at the different types of foraging problems. Section \ref{sec:second:natureinspired} discusses nature-inspired foraging algorithms while foraging algorithms that have not been inspired by nature are addressed in Section~\ref{standardforaging}. Section~\ref{multiforaging} discusses a more complex version of foraging with multiple object types. 
 
\subsection{Nature-inspired Swarm Robotics Foraging Algorithms}
\label{sec:second:natureinspired}

Foraging is an animal activity performed in order to retrieve food \cite{charnov1976optimal}. The next few sections describe in detail the algorithms that have been inspired by ant foraging, bee foraging and other animals in nature. 
