...
Foraging is a popular problem due to the vast opportunity of foraging for application to a variety of real-world problems from agriculture, construction to mining. The foraging problem has been adapted by many swarm robotics resources into a variety of flavours of foraging. Thus it is beneficial to present a taxonomy of robot foraging that can be used to classify existing research about foraging as well as contextualize the specific foraging problems addressed by this thesis. Multiple taxonomies have been developed as robot foraging research has progressed \cite{oster1978caste,ostergaard2001emergent}. However a more recent and complete foraging taxonomy is presented in \cite{winfield2009foraging}. The taxonomy classifies foraging solutions based on four major characteristics: the foraging environment, robots used for foraging, performance aspects, and the foraging strategy used. Each of these major characteristics have a number of minor characteristics which are outlined in Table~\cite{foragingtaxonomytable_part1}.  

The environment classification characteristic includes addressing aspects such as the type of search space, the type and placement of objects in the environment as well as the number of sinks. The robot classification characteristic refers to qualities such as  the number of robots, as well as sensory, power and actuation capabilities. The performance axis characterises robot foraging on the the goal of the research such as energy gained or time taken to forage. The strategy axis characterizes algorithms by how the algorithm goes about solving the foraging problem. 
