...
\Function{uniform}{$numberitems, \sigma, environmentsize$}
	\State \text{nonprioritizeditemsleft = floor((1-$\sigma$)*numberitems)}
	\State \text{prioritizeditemsleft = floor( $\sigma$*numberitems)}
...
\Function{gaussian}{$numberitems, \sigma, environmentsize$}
	...
	\State \text{prioritizeditemsleft = floor($\sigma$*numberitems)}
	\State \text{nonprioritizeditemsleft = floor((1-$\sigma$)*numberitems)}
	\State \text{deviation = environmentsize*$\sigma$/2}
	\While{\text{$prioritizeditemsleft > 0$}}
...
\Function{clustered}{$numitems, \sigma, environmentsize$}
  ...
  \State \text{Calculate total number of prioritized, $total_p = floor(\sigma*numitems)$}
  
  \State \text{Calculate total number of non-prioritized items, $total_{np} = floor((1-\sigma)*numitems)$}
