...
The honey bee algorithm specific parameters were selected based on \cite{seeley2009wisdom}, where $t_{wait}=200$ time steps, $f_{max}=100$ time steps, $\Phi=0.8$ and $\rho=0.1$. Testing the effect of each of the honey bee algorithm specific parameters was not in the scope of this thesis.
...
Let $R$ be the list containing all considered elements for environment item type ratio, $r$, and let $n_r$ be number of elements in the list. Let $T$ be the list containing all considered elements for initial swarm specialization ratio, $\tau(0)$, and let the number of elements in the list be $n_t$.

Let $Z$ be a matrix of dimensions $n_t \times n_r$. Let each entry of the matrix $Z_{ij}$ be the average foraging efficiency, $E_P$, over all experiments, for a specific environment ratio $r_j$, where $r_j$ is the $j$th element of $R$, and for a specific initial swarm specialization ratio $\tau_i$, such that $\tau_i(0)$ is the $i$th element of $T$.
...
The flexibility performance measures used in this study are based on the flexibility performance measures used in \cite{hecker2015beyond}, which have been adapted for the prioritized foraging problem, by only taking into account the prioritized item. The flexibility study addresses two aspects: (i) Flexibility with respect to prioritized item ratios, $r$, and (ii) flexibility with respect to different environment distributions.

\subsubsection{Flexibility over prioritized item ratio}
\label{setup:flexibility:prioritizeditemratio}

The goal is to determine how well the best value for swarm specialization ratio (where the best value is one that yields the greatest efficiency) for a specific environment item ratio, $r_u$, generalizes across all other environment item ratios, $r_v$, such that $u,v = 1,...,n_r$,$u \neq v$.

To do so, $I$ is defined as a list of size $n_r$, where each entry $I_j$ is the index $k$ that yields the maximum value for $Z_{ij}$, for $i = 1,..., n_\tau$, for a specific environment type ratio $j$. More succinctly:

\begin{equation}
I_j = ( k | Z_{kj} = max(Z_{ij}) \text{ for } i = 1,...,n_\tau)
\end{equation}

The normalized moment around the maximum foraging efficiency for a specific environment item type ratio $r_u$, is defined as follows: 
\begin{equation}
Fr_u = \sum_{v=1}^{n_r}(\dfrac{|Z_{I_uu}- Z_{I_uv}|}{Z_{I_uu}})
\end{equation}

And the final performance measure is the sum of the above normalized moments around the maximum foraging efficiency for every environment item type ratio:
...
This study evaluates flexibility, by analysing how swarm efficiency on various environment distributions (ED), described in Section~\ref{experimentenvironments}, is affected by swarm specialization ratio, $\tau_i(0)$ (described in Section~\ref{swarmparameters}).
