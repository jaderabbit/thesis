...
The obstacle avoidance technique was developed to be used in a continous environment. In order to adapt the technique for the 2D grid environment used in this thesis, the technique was discretized.
...
\Function{uniform}{$numberItems, r, S$}
	\State \text{nonPrioritizedItemsLeft = ceil((1-$r$)*numberItems)}
...
\Function{clustered}{$numitems, r, S$}
  ...
  \State \text{Calculate number of prioritized clusters, $clusters_p = ceil(r*total)$}    
  \State \text{Calculate number of non-prioritized clusters, $clusters_{np} = floor((1-r)*total)$}
  
  \State \text{Calculate number of prioritized items, $total_p = ceil(r*numitems)$}
