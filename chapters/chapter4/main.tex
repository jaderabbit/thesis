...
Let $R$ be the vector containing all considered elements for environment item type ratio, $r$, and let $n_r$ be the length of  $R$. Let $T$ be the vector containing all considered elements for initial swarm specialization ratio, $\tau(0)$, and let $n_t$ be the length of $T$.

Let $Z$ be a matrix of dimensions $(n_t, n_r)$. Let each entry of the matrix $Z_{ij}$ be the average foraging efficiency, $E_P$, over all experiments, for a specific environment ratio $r_j$, where $r_j$ is the $j$th element of $R$, and for a specific initial swarm specialization ratio $\tau_i$, such that $\tau_i$ is the $i$th element of $T$.
...
We need to determine how well the best value for swarm specialization ratio (where the best value is one that yields the greatest efficiency) for a specific environment item ratio, $r_u$, generalizes across all other environment item ratios, $r_v$, such that $u,v = 1,...,n_r$,$u \neq v$.

To do so, we first define $I$ as a vector of length $n_r$, where each entry $I_j$ is the index $k$ that yields the maximum value for $Z_{ij}$, for $i = 1,..., n_\tau$, for a specific environment type ratio $j$. More succinctly:

\begin{equation}
I_j = \{ k | Z_{kj} = max(Z_{ij}) \text{ for } i = 1,...,n_\tau\}
\end{equation}

We define a normalized moment around the max foraging efficiency for a specific environment item type ratio $r_u$, as follows: 
\begin{equation}
