...
A single \textbf{experiment} consists of 30 independant runs of an algorithm with a specific set of swarm parameters defined above, and on an environment which has a specific set of environmental parameters, defined above.
...
To do so, $I$ is defined as a list of length $n_r$, where each entry $I_j$ is the index $k$ that yields the maximum value for $Z_{ij}$, for $i = 1,..., n_\tau$, for a specific environment type ratio $j$. More succinctly:

\begin{equation}
I_j = \( k | Z_{kj} = max(Z_{ij}) \text{ for } i = 1,...,n_\tau\)
\end{equation}

The normalized moment around the max foraging efficiency for a specific environment item type ratio $r_u$, is defined as follows: 
\begin{equation}
F_u = \sum_{v=1}^{n_r}(\dfrac{|Z_{I_uu}- Z_{I_uv}|}{Z_{I_uu}})
\end{equation}
