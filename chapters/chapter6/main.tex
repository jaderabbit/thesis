...
% Examine flexibility of UNIFORM as a contrast to show the different behaviour and how that results in a higher perrformance? Maybe?
...
According to Figure~\ref{fig:problemscalability}, the Honeybee algorithm is the most scalable in terms of the problem, followed by the Na\"ive algorithm, with the Desert ant algorithm having the worst scalability. %WHY CAN I SAY THAT?
...
% The below statement does not explain it
%The line plotting expected scalability , runs on the assumption that efficiency would degrade proportionally to the total number of items that can be foraged, and plots the expected value for $PS$ accordingly. All algorithm's outperform the expected scalability. Rationally, one can motivate why that is the case, by understanding that environments with low density of items would require more time spent on exploration to find the items that can be foraging, which are more sparsely scattered around the environment. An environment with high density of items, despite having more items that require foraging, are more likely to have items closer to the sink and thus initially foraging could be fast. Figure~\ref{doesnotexistyet} supports the prior claim , by comparing the foraging performance of the Na\"ive algorithm, for each 200 time steps on a uniform environment with a sparse density $c=0.05$ compared to a uniform environment with a higher density $c=0.5$. 
